...
def draw_cube(ax, origin_x, origin_y, origin_z, size, edge_color=(0, 1, 0)):
    r = [0, size]
    vertices = [
...
def draw_colored_transparent_cube(ax, origin_x, origin_y, origin_z, size, color='red', alpha=0.3):
    ...
    ax.add_collection3d(art3d.Poly3DCollection(faces, facecolors=color, linewidths=0.7, edgecolors=(0, 0, 1), alpha=alpha))
...
# Générer la liste des nombres premiers jusqu'à 999999
def generate_primes(limit=999999):
    primes = []
    sieve = [True] * (limit + 1)
...
# Boucle pour gérer chaque nombre premier
for number in prime_numbers:
    ...
    draw_cube(ax, norm_cube2_x + norm_cube3_x, norm_cube2_y + norm_cube3_y, norm_cube2_z + norm_cube3_z, 0.1, edge_color=(1, 0, 0))  # Rouge pour cube1
    draw_cube(ax, norm_cube3_x, norm_cube3_y, norm_cube3_z, 1, edge_color=(0, 1, 0))  # Vert pour cube2
