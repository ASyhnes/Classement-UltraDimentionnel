...
# Dessiner les vecteurs entre tous les coins de tous les cubes du même type
def draw_complete_vectors_between_cubes(ax, cube_positions, size=1, color='blue', alpha=0.3):
...
# Listes pour stocker les positions des cubes rouges, verts et les points bleus
cube1_positions = []
cube2_positions = []
cube3_positions = []
point_positions = []
        ...
        try:
            ...
            if 0 <= number <= 999999999:
                ...
                for (x, y, z) in cube1.get_points():
                    final_x = x * 0.01 + norm_cube2_x + norm_cube3_x
                    final_y = y * 0.01 + norm_cube2_y + norm_cube3_y
                    final_z = z * 0.01 + norm_cube2_z + norm_cube3_z
                    ax.scatter(final_x, final_y, final_z, c='blue', marker='o')
                    ax.text(final_x, final_y, final_z, f"Point ({x},{y},{z})", size=10, zorder=1, color='k')
                    point_positions.append((final_x, final_y, final_z))
                ...
                cube1_positions.append((norm_cube2_x + norm_cube3_x, norm_cube2_y + norm_cube3_y, norm_cube2_z + norm_cube3_z))
...
# Dessiner les vecteurs entre tous les points dans cube1
if len(point_positions) > 1:
    draw_complete_vectors_between_cubes(ax, point_positions, size=0.01, color='blue', alpha=0.5)

# Dessiner les vecteurs entre tous les cubes rouges (cube1)
if len(cube1_positions) > 1:
    draw_complete_vectors_between_cubes(ax, cube1_positions, size=0.1, color='red', alpha=0.5)
